...
public class MySQL {
    ...
    public MySQL(ToonHQ toonHQ){
        this.toonHQ = toonHQ;
        host = toonHQ.getProperties().getString("mysql_host");
        username = toonHQ.getProperties().getString("mysql_username");
        password = toonHQ.getProperties().getString("mysql_password");
        database = toonHQ.getProperties().getString("mysql_database");
        port = toonHQ.getProperties().getInt("mysql_port");
    }
     ...
     */
    public boolean update(String sql, Object... updates){
        PreparedStatement prep = null;
        try{
            prep = connection.prepareStatement(sql);
            for(int i = 0; i < updates.length; i++){
                prep.setObject(i+1, updates[i]);
            }
            prep.executeUpdate();
            return true;
        }catch(SQLException e){
